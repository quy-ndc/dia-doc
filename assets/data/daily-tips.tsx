...
export type DiabeticTip = {
    title: string
    icon: ReactNode
    desc: string
}

export type DiabeticTipsRecord = Record<number, DiabeticTip>

const diabeticTips: DiabeticTipsRecord = {
    1: {
...
export function getRandomDiabeticTip(): DiabeticTip {
    const tipIds = Object.keys(diabeticTips).map(Number)
    const randomId = tipIds[Math.floor(Math.random() * tipIds.length)]
    return diabeticTips[randomId]
}
