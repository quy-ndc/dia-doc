...
import { useCarePlanInstanceDoctor, useUserHealthCarePlan } from '../../../service/query/user-query'
...
export default function DoctorManageTodayCarePlanScreen() {
    const { id } = useLocalSearchParams()
    const [refreshing, setRefreshing] = useState(false)

    const { data, isLoading, isError, refetch, remove } = useQuery({
        ...useCarePlanInstanceDoctor({
                    ...
                    <View className='w-full flex-col gap-3'>
                        ...
                        {isLoading ? (
                            ...
                            <FlashList<HealthCarePlan>
                                data={items}
                                renderItem={({ item }) => (
                                    <HealthcarePlanItem item={item} display='manage' patientId={id as string} />
                                )}
